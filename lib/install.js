...
function getDownloadUrl(version, platform, arch) {
  ...
  // GoReleaser creates files with version number in the filename
  const filename = `herald_${version}_${platform}_${arch}.${ext}`;
    ...
    https
      .get(url, (response) => {
        ...
        if (response.statusCode !== 200) {
          reject(
            new Error(
              `Download failed with status: ${response.statusCode}. URL: ${url}`
            )
          );
          return;
        }
...
async function install() {
  try {
    ...
    const version = process.env.HERALD_VERSION || packageJson.version;
